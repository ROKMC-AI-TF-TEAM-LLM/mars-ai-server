...
1. **route** — 질문 + 대화 이력 → `rewritten_query` + `intent`를 한 번의
   구조화 호출(ClassifyAndRewrite)로. 멀티턴 맥락 해소 + 구어체 정규화 +
   처리 경로 분류. 실패 시 원본 질문 + DOC_SEARCH 폴백.
   intent가 도구 레지스트리(tools.py) 키면 해당 도구 노드로 직행
   (예: SMALLTALK — 검색·검증 없이 직접 응답, sources 비움, grounded=False).
   요청의 tool 필드가 intent를 강제하면 분류를 건너뛰고 재작성만 수행한다

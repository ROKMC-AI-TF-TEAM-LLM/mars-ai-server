...
```python
def 노드이름(state: RetrievalState) -> dict:
    값 = state.get("어떤_키")          # 읽기
...
### 1-2. RetrievalState의 진화 — 질문 하나를 따라가 보면

`retrieval_graph/state.py`의 TypedDict가 전체 필드 목록이다.
...
그래프: `START → chunk → embed_and_upsert → END`
...
## 4. retrieval_graph — 질문 하나가 답변이 되기까지

그래프 (graph.py `_build_graph()`):

```
START → route ─(SMALLTALK)→ smalltalk → END
          └→ dense_retrieve → bm25_retrieve → fuse → rerank → generate → verify
                                  (조건부) finalize → END
                                            increment_retry → generate (재실행)
                                            fallback → END
```

...
모듈 상수 두 개도 여기 있다:
- `DOMAINS = ("HR", "TECH", "FINANCE_LEGAL", "GENERAL", "MANUAL", "DIRECTIVE")` —
  문서 도메인 (MANUAL=교범, DIRECTIVE=훈령). Milvus `domain` 필드 값이자
  요청 `domain` 필터의 허용 값. **라우터 분류와 무관** — 검색 필터는
  요청의 requested_domain만 쓴다.
...
- `flush()` — insert를 세그먼트로 확정. `rebuild_bm25()`가 조회 전에 호출.
- `fetch_all_children(output_fields)` — BM25 재빌드·문서 인벤토리용 전체 조회.
  query 1회 상한(16,384행)을 넘는 코퍼스를 위해 **query_iterator로 전체 순회**,
  미지원 환경(구버전/Lite 제약)은 단일 query 폴백(경고 로그).
- `list_documents()` — source_doc별 도메인/공개범위/적재시각 집계
  (GET /documents 인벤토리의 재료, 문서명 오름차순).
...
- `_load_cached()` — 빌드마다 새로 쓰는 **version 파일(uuid)** 로 갱신을
  감지해 바뀌었으면 다시 읽는다 (mtime은 연속 재빌드 시 같은 값이 나와
  갱신 감지가 샐 수 있어 uuid로 교체). `lru_cache`로 영구 캐시하면 안 되는
  이유(실측 보안 버그): reindex 스크립트(별도 프로세스)가 문서를 DEPT_ONLY로
  바꿔도 서버 메모리에는 옛 visibility가 남아 **타 부서에 문서가 노출**됐다.
...
```python
embeddings = _embed_texts([c["text"] for c in chunks])  # ① 임베딩 (16개씩 배치 HTTP —
                                                        #    CPU에서 64배치 timeout 실측)
parent_store.insert_parents(parents)                    # ② 부모 먼저 insert
inserted = vectorstore.insert_children(rows)            # ③ 자식 insert
rebuild_bm25()                                          # ④ flush 후 BM25 전체 재빌드
```
...
scope = state.get("requested_domain") or "GENERAL"           # 요청 명시 도메인만 한정
expr = build_acl_filter_expr(scope, user_department)         # ACL → Milvus 필터식
embedding = _embed_query(query)                              # 임베딩 서버 호출
hits = get_client().search(..., filter=expr, limit=SEARCH_TOP_K, ...)
```
...
```python
raw = bm25_search(query, top_k=SEARCH_TOP_K * 3)       # 3배 오버샘플
filtered = filter_by_acl(raw, scope, user_department)  # ★ 코드 레벨 ACL (우회 금지)
return {"bm25_candidates": filtered[:SEARCH_TOP_K]}
```

- BM25 인덱스는 Milvus 밖에 있어서 DB 필터가 못 미친다. 그래서
  `filter_by_acl()` 후처리가 **필수**다 (CLAUDE.md 보안 규칙).
- 필터로 깎일 것을 감안해 3배 깊이로 검색한 뒤 필터 → SEARCH_TOP_K로 자른다.
...
```
1차: bind_tools([schema], tool_choice=스키마명), max_tokens=512
     → response.tool_calls 파싱
     (상한을 두는 이유: 모델이 분류 대신 질문에 장문으로 답하는 폭주를
      토큰 수준에서 차단 — 46초 생성 실측)
      ↳ 실패 시
2차: 본문에서 { ... } 블록을 정규식으로 찾아 pydantic 스키마로 검증
     (실패 시 원문 일부를 WARNING 로그 — L40 성공률 측정 재료)
      ↳ 실패 시
3차: response_format={"type": "json_object"}, max_tokens=256으로 1회 재호출.
     프롬프트에는 스키마 정의가 아니라 **예시 JSON**을 준다 — 스키마
     properties를 보여주면 7B가 타입 정의를 값처럼 복사한다 (실측).
     스키마 클래스의 RETRY_EXAMPLE(ClassVar)이 있으면 그것을, 없으면
     타입별 자리표시로 합성. bool 자리표시는 False (앵무새 복사돼도
     fail-closed 방향)
      ↳ 전부 실패하면 None
```
...
- **bulk_ingest.py** — 디렉터리의 .md/.txt/.pdf를 순회하며 indexer_graph 호출.
  `.md`는 `##` 헤딩으로 섹션 분해, `.txt`는 통짜, `.pdf`는 pdfplumber로
  추출해 통짜 (스캔본 미지원). HWP/DOCX는 미확정 항목이라 아직 미지원.
  ```
...
- `tests/unit_tests/` (187개) — 외부 서비스 불필요. `make test`.
...
| 검색 깊이 | `.env` `SEARCH_TOP_K` (dense/bm25 공통) | 리랭커 비용 증가 |
...
2. **레지스트리 등록** — tools.py에 두 줄:
   ...
   ```
   그래프 배선(graph.py — **복합 계획 순차 실행·합성 포함**), 라우터 분류
